"""Per-ticker daily OHLCV price store builder.
...
re-running only appends genuinely new trading days. Pass ``rebuild=True`` to
recompute every row from scratch (e.g. after an intentional basis refresh).

This module is the reusable home for the logic; the
``scripts/build_daily_price_store.py`` entry point is a thin CLI wrapper over
it. The on-disk CSV and ``_manifest.json`` byte layout is a consumer contract
(``eit-research`` reads it) and must remain stable.
"""
...
def _make_price_provider(market: str):  # noqa: ANN202 - returns one of two provider types
    if market == "kr":
...
def build_daily_price_store(
    *,
    ...
    end_date = end or _last_business_day(datetime.now(UTC).date())
